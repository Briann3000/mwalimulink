<?php
// public_school_search.php - Modern Public School Directory
$per_page = 20;
...
$schools = !empty($whereSql)
    ? R::find('public_school', "$whereSql ORDER BY name LIMIT ? OFFSET ?", $queryParams)
    : R::find('public_school', 'ORDER BY name LIMIT ? OFFSET ?', [$per_page, $offset]);
            ...
            <form method="GET" action="/schools/public" style="margin: 0;">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; align-items: flex-end;">
                    <div>
                        ...
                        <input type="text" name="search" value="<?= h($search) ?>" placeholder="School name or constituency..." style="width: 100%; box-sizing: border-box; margin: 0;">
                    </div>
                    ...
                    <div style="display: flex; gap: 6px;">
                        ...
                        <?php if ($search || $countyFilter || $levelFilter): ?>
                            <a href="/schools/public" style="height: 40px; padding: 0 14px; background: #f1f5f9; color: #475569; border-radius: 6px; font-weight: 600; font-size: 0.82rem; display: inline-flex; align-items: center; text-decoration: none;">Reset</a>
                        <?php endif; ?>
                    </div>
                </div>
            </form>
