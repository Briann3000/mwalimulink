...
<form method="get" action="index.php">
    ...
    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" 
           placeholder="Search by Name, Level, County or Constituency">
    <button type="submit">Search</button>
</form>

<p>Showing <?= count($schools) ?> of <?= $total_pages * 20 ?> schools<?= $search ? " for <b>".htmlspecialchars($search)."</b>" : "" ?></p>
<!-- LIST PRIMARY & SECONDARY by kesh -->
<!--<small><i style="font-size:1vw;"><?php // $numPrimarySchools = R::count('public_school', 'level = ?', ['PRIMARY SCHOOL']);
// $numSecondarySchools = R::count('public_school', 'level = ?', ['SECONDARY SCHOOL']);
...
<div class="pagination">
    <?php if ($page > 1): ?>
        <a href="?action=public_school_search&search=<?= urlencode($search) ?>&page=<?= $page-1 ?>">&laquo; Prev</a>
    <?php endif; ?>
    ...
    <?php for ($p = 1; $p <= $total_pages; $p++): ?>
        ...
        <?php elseif ($p == 1 || $p == $total_pages || abs($p - $page) <= 2): ?>
            <a href="?action=public_school_search&search=<?= urlencode($search) ?>&page=<?= $p ?>"><?= $p ?></a>
    ...
    <?php if ($page < $total_pages): ?>
        <a href="?action=public_school_search&search=<?= urlencode($search) ?>&page=<?= $page+1 ?>">Next &raquo;</a>
    <?php endif; ?>
</div>
