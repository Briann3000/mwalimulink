...
<div class="workspace-wrapper">
    ...
    <main class="content-pane"
        style="<?= !$authUser ? 'max-width: 1400px; width: 100%; margin: 0 auto; padding: 2rem 1.5rem; box-sizing: border-box;' : '' ?>">
        <div style="margin-bottom: 1.5rem;">
            <h2 style="margin: 0 0 4px; font-size: 1.4rem; color: #0f172a;">🏫 Private Schools & Academies Directory
            </h2>
        ...
        <div
            ...
            <form method="GET" action="/schools/private" style="margin: 0;">
                ...
                <div
                    style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 10px; align-items: flex-end;">
                    <div>
                        <label
                            style="font-size: 0.75rem; font-weight: 700; color: #64748b; text-transform: uppercase; margin-bottom: 4px; display: block;">Keywords</label>
                        <input type="text" name="search" value="<?= h($search) ?>"
                            placeholder="Academy name or constituency..."
                            style="width: 100%; box-sizing: border-box; margin: 0; height: 38px;">
                    </div>
                    ...
                    <div>
                        <label
                            style="font-size: 0.75rem; font-weight: 700; color: #64748b; text-transform: uppercase; margin-bottom: 4px; display: block;">Level
                            / Grade</label>
                        <select name="level" style="width: 100%; box-sizing: border-box; margin: 0; height: 38px;">
                            <option value="">All Categories</option>
                            <option value="PRIMARY" <?= ($levelFilter === 'PRIMARY') ? 'selected' : '' ?>>Primary / Academy
                                (CBC)</option>
                            <option value="SECONDARY" <?= ($levelFilter === 'SECONDARY') ? 'selected' : '' ?>>Secondary /
                                High School</option>
                            <option value="JUNIOR" <?= ($levelFilter === 'JUNIOR') ? 'selected' : '' ?>>Junior Secondary
                            </option>
                        </select>
                    </div>
                    ...
                    <div>
                        <label
                            style="font-size: 0.75rem; font-weight: 700; color: #64748b; text-transform: uppercase; margin-bottom: 4px; display: block;">Sort
                            By</label>
                        <select name="sort" style="width: 100%; box-sizing: border-box; margin: 0; height: 38px;">
                            <option value="name_asc" <?= ($sort === 'name_asc') ? 'selected' : '' ?>>School Name (A &rarr;
                                Z)</option>
                            <option value="name_desc" <?= ($sort === 'name_desc') ? 'selected' : '' ?>>School Name (Z
                                &rarr; A)</option>
                            <option value="county_asc" <?= ($sort === 'county_asc') ? 'selected' : '' ?>>County (A &rarr;
                                Z)</option>
                            <option value="county_desc" <?= ($sort === 'county_desc') ? 'selected' : '' ?>>County (Z &rarr;
                                A)</option>
                            <option value="level_asc" <?= ($sort === 'level_asc') ? 'selected' : '' ?>>Level (A &rarr; Z)
                            </option>
                            <option value="constituency_asc" <?= ($sort === 'constituency_asc') ? 'selected' : '' ?>>
                                Constituency (A &rarr; Z)</option>
                        </select>
                    </div>

                    <div style="display: flex; gap: 6px;">
                        <button type="submit" class="btn-primary"
                            style="height: 38px; flex: 1; margin: 0; display: inline-flex; align-items: center; justify-content: center; gap: 6px;">
                            <i class="fa fa-filter"></i> Apply
                        </button>
                        <?php if ($search || $countyFilter || $levelFilter || $sort !== 'name_asc'): ?>
                            <a href="/schools/private?per_page=<?= $per_page ?>"
                                style="height: 38px; padding: 0 14px; background: #f1f5f9; color: #475569; border-radius: 6px; font-weight: 600; font-size: 0.82rem; display: inline-flex; align-items: center; justify-content: center; text-decoration: none; border: 1px solid #cbd5e1; box-sizing: border-box;">Reset</a>
                        <?php endif; ?>
                    </div>
                </div>
            </form>
        ...
        <div
            ...
            <span style="font-size: 0.85rem; color: #64748b;">
                Showing <strong><?= number_format($from_count) ?></strong> to
                <strong><?= number_format($to_count) ?></strong> of <strong><?= number_format($total) ?></strong>
                institutions
            </span>
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <select onchange="location.href=this.value"
                    style="padding: 4px 10px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 0.82rem; background: white; margin: 0; width: auto; height: 34px;">
                    <option value="<?= $basePerPageUrl ?>per_page=20" <?= ($per_page == 20) ? 'selected' : '' ?>>20 per
                        page</option>
                    <option value="<?= $basePerPageUrl ?>per_page=50" <?= ($per_page == 50) ? 'selected' : '' ?>>50 per
                        page</option>
                    <option value="<?= $basePerPageUrl ?>per_page=100" <?= ($per_page == 100) ? 'selected' : '' ?>>100 per
                        page</option>
                </select>
            </div>
        </div>
                            ...
                            <?php foreach ($schools as $school): ?>
                                <tr>
                                    <td style="font-weight: 600;">
                                        <a href="/schools/private/detail?id=<?= $school->id ?>"
                                    ...
                                    <td>
                                        <span
                                            style="background: #f0fdf4; color: #166534; padding: 3px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: 700; border: 1px solid #bbf7d0;">
